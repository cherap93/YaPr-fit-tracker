...
// distance возвращает дистанцию, которую преодолел пользователь (км)
...
func (r Running) Calories() float64 {
	if r.Duration.Minutes() == 0 {
		return 0
	}
	res := ((18 * r.meanSpeed() + 1.79) * r.Weight / MInKm * (r.Duration.Minutes() / 60) * MinInHours)
	return res
}
...
func (w Walking) Calories() float64 {
	...
	k1 := CaloriesWeightMultiplier
	k2 := CaloriesSpeedHeightMultiplier
	res := ((k1 * w.Weight + ((speedInMSec*speedInMSec) / heightInM) * k2 * w.Weight) * (w.Training.Duration.Minutes()/60) * MinInHours)
	return res
}
...
func (s Swimming) meanSpeed() float64 {
	...
	return float64(s.LengthPool * s.CountPool) / MInKm / (s.Duration.Minutes() / 60) 
}
...
func (s Swimming) Calories() float64 {
	res := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * (s.Duration.Minutes() / 60) 
	return res
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	message.Distance = float64(s.LengthPool) * float64(s.CountPool) / 1000
	return message
}
